...
to (a listener a HOST program owns — distinct from the box's own **door**, which
dabs owns and the `publish:` grant opens), each `socket:` (the host path, expanded like a source origin: `~`, `$VAR`,
