...
var commandDocs = map[string]cmdDoc{
	...
	"services": {Help: "list the services the boxes publish — name, type, box and instance, the host address `services serve` gives it, and its state (up | down | conflict, when another box owns the name). `services serve` forwards each one from a stable 127.0.0.1 port (a name keeps its port across boxes) and serves an index of them at 127.0.0.1:28080 until interrupted. A box publishes by running the in-box forwarder: `forward publish <name> --type webui|general --port <n>`", Args: "services [serve]"},
