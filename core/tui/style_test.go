...
// path a script capturing `dabs recipe --no-command` output hits. On a real terminal the same
// helpers would prepend the glyph and wrap the text in ANSI color; here they
// must not, so machine parsing (`dabs recipe --no-command | awk '{print $1}'`) is not corrupted.
