...
A box gets a network namespace of its own, but a recipe that says nothing about
`egress` gets full outbound access through it — so an unconfined box can phone
...
an optional `http_proxy` content chain. See [Egress](/guides/egress).
...
([recipe schema](/reference/recipe-schema)). On Linux (the bwrap driver)
...
than handing the box your host's network. pasta must be snapshot `2025_05_03` or newer, for the address flags dabs passes: Debian trixie+ and Fedora 41+ package a new enough one, Ubuntu's and Alpine's current packages refuse those flags, and there you build from [passt.top](https://passt.top) at the version `contrib/recipes/dabseption.Dockerfile` pins.
