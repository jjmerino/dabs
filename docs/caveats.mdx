...
an error. `--detach` also needs a driver whose box carries a process of its own
(docker, apple); on one that enters its box afresh per command (bwrap) it is
refused rather than quietly run in the foreground.
