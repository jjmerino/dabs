...
driver) `egress: open` is built by **pasta** — install the `passt` package
(`apt install passt`, `dnf install passt`, `apk add passt`) and run dabs as an
unprivileged user; a boot that cannot have pasta refuses and says so instead of
handing the box your host's network.</Info>
