...
```bash
dabs recipe sh --name dev --no-command
```
...
```text
  ...
  repo-________  project                      main                      ~/repo
  ├─ login-fix   worktree          ●          no-diff (dabs/login-fix)  ~/.dabs/nodes/login-fix/held/worktree
  └─ dev         box (bwrap)                  live                      dabs exec shell-____________ bash
```

## cd into a node

`dabs cd <node>` prints a node's **working place** as a bare path, resolved per
kind: a project to its source repo, a worktree to its checkout, a box to its own
node dir. For a project and a worktree that is the path INFO shows above; for a
**box** it is not — a box has no working directory, so its INFO cell carries the
`dabs exec … bash` line that shells into it, while `cd` gives you the node dir on
the host. A child process cannot move its parent's shell, so you wrap it:
...
and an ambiguous one is refused rather than guessed. A box's node dir holds its
three spaces as subdirectories — `volume/`, `held/`, `tmp/` (see
[Spaces](/guides/spaces)); `dabs cd login-fix` lands in the worktree's checkout
instead, which is that node's own `held/worktree`.
