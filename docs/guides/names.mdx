...
`dabs cd <node>` prints a node's **working place** as a bare path — the same
place the INFO column shows, resolved per kind: a project to its source repo, a
worktree to its checkout, and a box to its own node dir. A child process cannot
move its parent's shell, so you wrap it:
