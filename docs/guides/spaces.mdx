...
```text
  ...
  └─ spacey-________  box (bwrap)  ●    ●     ●    live   dabs exec shell-____________ bash
```

`dabs info <node>` says the same thing one node at a time, in words
(`holds files` / `empty`) — and names the recipe that mounted each space.
...
│   spacey-________  box (bwrap)  ●    ●     ●    live        │
