...
One node's full model, where `ls` gives every node a row each. The node resolves
...
- **appended** — the argv `dabs recipe <name> <cmd…>` appended to the recipe's
  ...
  `dabs ls`: an appended command can carry a prompt or a secret, and a listing
  of everything dabs owns is the wrong place to spill either.
